...
public class GetConfig {
    ...
    static final AbstractMap<String, String> configurations = new LinkedHashMap<>();
    ...
    public static <T> T pull(String config){
        ...
        if(configurations.containsKey(config) && life < cacheLife){
            return (T)gson.fromJson(configurations.get(config),new TypeToken<T>() {}.getType());
        }
        ...
        try {
            ...
            configurations.put(config,configFile.toString());
        }
        catch(Exception e){
            return null;
        }
